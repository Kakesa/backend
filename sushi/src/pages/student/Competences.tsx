...
import { apiGetAllCompetences, apiGetStudentEvaluations, apiGetCompetenceProgress } from "@/services/api/competences.api";
import { Subject, Competence, StudentCompetenceEvaluation, CompetenceProgress } from "@/types";
import { Loader2, Target, Award, TrendingUp, CheckCircle2, Circle, AlertCircle, BookOpen, Star } from "lucide-react";
...
export default function StudentCompetences() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const [subs, comps, evals] = await Promise.all([
          ...
          apiGetStudentEvaluations(user.linkedId, selectedTrimester === "all" ? undefined : parseInt(selectedTrimester))
        ]);
        ...
        const progressResults = await Promise.all(
          comps.map(c => user.linkedId ? apiGetCompetenceProgress(user.linkedId, c.id) : Promise.resolve(null))
        );
    ...
    loadData();
  }, [user?.linkedId, selectedTrimester]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Suivi de votre progression par objectifs
          </p>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-yellow-500">
                  {evaluations.filter(e => e.level === "en_cours").length}
                </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-purple-500">
                  {evaluations.filter(e => e.level === "expert").length}
                </p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Votre avancement dans l'acquisition des compétences</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <CardContent>
            <div className="space-y-6">
              {subjectComps.map((comp) => {
                ...
                const compEvaluations = evaluations.filter(e => e.competenceId === comp.id);
                ...
                return (
                  <div key={comp.id} className="border rounded-lg p-4">
                    ...
                    <div className="space-y-2">
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {evaluation.level === "expert" && <Star className="h-4 w-4 text-yellow-500" />}
                              </div>
                            </div>
                          );
                        })}
                      ...
                      {comp.objectives.filter(obj => !compEvaluations.some(e => e.objectiveId === obj.id)).length > 0 && (
                        <div className="mt-3 pt-3 border-t">
                          <p className="text-xs font-medium text-muted-foreground mb-2">Objectifs à évaluer :</p>
                          <div className="space-y-1">
                            {comp.objectives.filter(obj => !compEvaluations.some(e => e.objectiveId === obj.id)).map(obj => (
                              <div key={obj.id} className="flex items-center gap-2 p-2 rounded bg-muted/30">
                                <Circle className="h-3 w-3 text-muted-foreground" />
                                <div className="flex-1">
                                  <p className="text-xs">{obj.name}</p>
                                </div>
                                <Badge variant="outline" className="text-[10px]">
                                  {obj.level === "beginner" ? "Débutant" : 
                                   obj.level === "intermediate" ? "Intermédiaire" : "Avancé"}
                                </Badge>
                              </div>
                            ))}
                          </div>
        ...
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Les compétences n'ont pas encore été configurées pour votre classe.
            </p>
          </CardContent>
        </Card>
      )}
